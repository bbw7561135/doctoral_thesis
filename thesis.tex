...
\chapter{\code{gadfly}: A pandas-based Framework for Analyzing \textsc{gadget} Simulation Data}
...
\begin{figure}
\begin{center}
\includegraphics[width=\columnwidth]{figures/code_usage/code_usage}
...
W(r,h_{\mathrm s}) =
     \begin{cases}
       1 - 6 \left( \frac{r}{h_{\mathrm s}} \right)^2 + 6 \left( \frac{r}{h_{\mathrm s}} \right)^3, & 0 \leq \frac{r}{h_{\mathrm s}} \leq \frac{1}{2}\\
       2 \left(1 - \frac{r}{h_{\mathrm s}}\right)^3, & \frac{1}{2} < \frac{r}{h_{\mathrm s}} \leq 1\\
       0, & \frac{r}{h} >  1,\\
     \end{cases}
\end{equation}
where $r$ is the radius and $h_{\mathrm s}$ is the characteristic width of the kernel, otherwise known as the smoothing length.  The physical density at any point, $\rho(\mathbf{r})$, is then represented by the sum over all particles
\begin{equation}
\rho({\mathbf r}) \simeq \sum_j m_j W({\mathbf r} - {\mathbf r}_j, h_{\mathrm s}),
\end{equation}
